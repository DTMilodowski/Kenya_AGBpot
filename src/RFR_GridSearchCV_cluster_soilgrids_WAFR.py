'''
17/10/2018 -DTM
updates predominately focussed on the selection of training data
bassed on k-means clustering of the explanatory variables
...
# open 1992 file to set up template
lc1992file  = gdal.Open('%s/esacci/ESACCI-LC-L4-LCCS-Map-P1Y-1992-v2.0.7-1km-mode-lccs-class-%s.tif' % (path,country_code))
lc = lc1992file.GetRasterBand(1).ReadAsArray()
trainingmask=np.ones(lc.shape)
print('total number of pixels, N = %i' % trainingmask.sum())
trainingmask = (lc>=50)*(lc<=180)
lc1992file = None

# loop through years and only keep pixels tha do not change from year to year
lc_start = 1993
lc_end = 2015
print('training sample, N = %i' % trainingmask.sum())
for yy in range(lc_start,lc_end+1):
    lc_file = gdal.Open('%s/esacci/ESACCI-LC-L4-LCCS-Map-P1Y-%i-v2.0.7-1km-mode-lccs-class-%s.tif' % (path,yy,country_code))
    lc_p = lc.copy()
    lc = lc_file.GetRasterBand(1).ReadAsArray()
    trainingmask *= (lc==lc_p)
    print('training sample, N = %i' % trainingmask.sum())

"""
# now buffer the forest training areas by a 7 pixel diameter neighbourhood
temp1,temp2 = np.meshgrid(np.arange(-3.,4.),np.arange(-3.,4.))
buffer = np.sqrt(temp1**2+temp2**2)<=3.
frst = filt.minimum_filter(frst,footprint=buffer,mode='reflect')
"""
...
#set target
target = np.zeros(trainingmask.shape)-9999.
target[trainingmask==True] = agbdata[trainingmask==True]
...
# extract data for final prediction here, n
"""
# using kenya mask
slcpred = kenya*wc2mask*(lc2015!=210)*soilmask
"""
slcpred = wc2mask*(lc!=210)*soilmask
...
# first undertake cluster analysis to extract "soil-climate zones" from which to
# draw final training sample
climsoil_cluster = KMeans(n_clusters=20, random_state=500).fit_predict(X)
climsoil_cluster = KMeans(n_clusters=20, random_state=500).fit_predict(StandardScaler().fit_transform(X))
clusters = np.zeros(slc.shape)*np.nan
clusters[slc]=climsoil_cluster
plt.imshow(climsoil_cluster,cmap='tab20'); plt.savefig('cluster.png');plt.show()

# now loop through the clusters, and extract the top 20% of the biomass values
# from each cluster
clusters_v2 = clusters.copy()

for cc in range(0,20):
    threshold = np.percentile(agbdata[clusters==cc],90)
    print(np.all((clusters==cc,agbdata<threshold),axis=0).sum())
    clusters_v2[np.all((clusters==cc,agbdata<threshold),axis=0)]=np.nan
    print(threshold)
